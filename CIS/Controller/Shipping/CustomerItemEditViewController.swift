...
class CustomerItemEditViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, CustomCellDelegate, UINavigationControllerDelegate, UITextFieldDelegate, ItemTextFieldDelegate {
    ...
    @IBAction func saveCustomerItem(_ sender: Any) {
        ...
        if(customerNameTextField.text!.trimmingCharacters(in: .whitespacesAndNewlines) == "") {
            ...
            let alertController = UIAlertController(title: "请填写正确数据", message: "请填写物品信息", preferredStyle: .alert)
            let alertAction = UIAlertAction(title: "OK", style: .default, handler: nil)
            alertController.addAction(alertAction)
            present(alertController, animated: true, completion: nil)
        } else {
    ...
    func deleteCell(cell: UITableViewCell) {
        ...
        if let deletionIndexPath = customerItemTableView.indexPath(for: cell) {
            ...
            let itmMO = imageMOStructArray[deletionIndexPath.section].itemMOStructArray[deletionIndexPath.row].itemMO
    ...
    @objc func deleteImage(sender:UIButton)
    {
        ...
        let context = appDelegate.persistentContainer.viewContext
        let imgMO = imageMOStructArray[sender.tag].imageMO
        customerMO!.removeFromImages(imgMO)
        imgMO.removeFromCustomers(customerMO!)
    ...
    func addStockItem(_ itemMO: ItemMO) {
        ...
        let newImageMO = ImageMO(context: context)
        newImageMO.imageFile = itemMO.image!.imageFile
        newImageMO.shipping = shippingMO
        ...
        newImageMO.addToCustomers(customerMO!)
        customerMO!.addToImages(newImageMO)
